...
import { expect as playExpect, ExtensionCardPage, RunnerOptions, test, ResourceConnectionCardPage, ExtensionState } from '@podman-desktop/tests-playwright';
...
test.use({ 
  runnerOptions: new RunnerOptions({ customFolder: 'minc-extension-tests-pd', autoUpdate: false, autoCheckUpdates: false }),
});
test.beforeAll(async ({ runner, page, welcomePage }) => {
  runner.setVideoAndTraceName('minc-extension-e2e');
...
test.describe.serial('MINC extension verification', () => {
  test.describe.serial('MINC extension installation', () => {
    ...
    test('Extension (card) is installed, present and active', async ({ navigationBar }) => {
      ...
      await playExpect.poll(async () => 
        await extensions.extensionIsInstalled(extensionLabelMinc), { timeout: 30000 },
      ).toBeTruthy();
  ...
  test.describe.serial('MINC extension handling', () => {
    test('Extension can be disabled', async ({ navigationBar }) => {
      const extensions = await navigationBar.openExtensions();
      await playExpect.poll(async() => await extensions.extensionIsInstalled(extensionLabelMinc), { timeout: 30000 }).toBeTruthy();
      const extensionCard = await extensions.getInstalledExtension(extensionLabelNameMinc, extensionLabelMinc);
      await playExpect(extensionCard.status).toHaveText(ExtensionState.Active);
...
async function removeExtension(navBar: NavigationBar): Promise<void> {
  ...
  await playExpect.poll(async () => await extensions.extensionIsInstalled(extensionLabelMinc), { timeout: 15000 }).toBeFalsy();
}
