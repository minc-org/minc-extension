    ...
    test.describe
      .serial('MINC cluster deployment workflow', () => {
        ...
        test('Create MINC cluster', async ({ page, navigationBar }) => {
          ...
          await playExpect(
            mincClusterForm.getByRole('textbox', { name: 'HTTP Port for the routes' }),
          ).toHaveValue('80');
          await playExpect(
            mincClusterForm.getByRole('textbox', { name: 'HTTPS Port for the routes' }),
          ).toHaveValue('443');
          ...
          await playExpect(mincResourcesCard.resourceElementConnectionStatus).toBeVisible(); // BUG: sometimes not appearing in resources after creating?
        ...
        test('Deploy container to MINC cluster', async ({ page, navigationBar }) => {
          ...
          await playExpect.poll(async () => await imagesPage.waitForImageExists(IMAGE_NAME), { timeout: 10_000 }).toBeTruthy();
          ...
          await deployToKubernetesPage.deployPod(POD_NAME, { useKubernetesServices: true, isOpenShiftCluster: true }, KUBERNETES_CONTEXT); //flaky?
        ...
        test('Delete MINC cluster', async ({ page, navigationBar }) => {
          ...
          await playExpect(
            mincResourcesCard.card.getByRole('region', { name: 'markdown-content' }),
